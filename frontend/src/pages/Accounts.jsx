...
export default function Accounts() {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "balance",
      render: (v) => (
        <span style={{ fontWeight: 600, color: "#52c41a" }}>{fmt(v)} ₫</span>
      ),
    },
    {
      title: "Trạng thái",
      ...
      key: "status",
      render: (v) => (
        <Badge status={v === "ACTIVE" ? "success" : "error"} text={v} />
      ),
    },
    {
      title: "Thao tác",
      ...
      render: (_, r) => (
        <Space>
          <Button
            size="small"
            icon={<EyeOutlined />}
            onClick={() => showBalance(r.accountId)}
          >
            Số dư
          </Button>
          <Button
            ...
            onClick={() => showHistory(r.accountId)}
          >
            Lịch sử
          </Button>
          <Button
            size="small"
            onClick={() => toggleStatus(r.accountId, r.status)}
          >
    ...
    { title: "ID", dataIndex: "transactionId", key: "id", width: 50 },
    {
      ...
      render: (v) => {
        const colors = {
          ...
          TRANSFER_IN: "cyan",
          TRANSFER_OUT: "orange",
          INTER_BRANCH_IN: "purple",
          INTER_BRANCH_OUT: "magenta",
        };
  ...
  return (
    <div className="fade-in">
      ...
      <Card>
        <Space style={{ marginBottom: 16 }}>
          ...
          <Button
            ...
            onClick={() => {
              loadCustomers();
              form.setFieldsValue({ branchId: branch });
              setCreateOpen(true);
            }}
          >
            Mở tài khoản
        ...
        {balanceModal && (
          <Descriptions bordered column={1}>
            <Descriptions.Item label="Tài khoản">
              #{balanceModal.accountId}
            </Descriptions.Item>
            <Descriptions.Item label="Chủ TK">
              {balanceModal.customerName}
            </Descriptions.Item>
            <Descriptions.Item label="Chi nhánh">
              <Tag color="blue">{balanceModal.branchId}</Tag>
            </Descriptions.Item>
            ...
            <Descriptions.Item label="Trạng thái">
              <Badge
                status={balanceModal.status === "ACTIVE" ? "success" : "error"}
                text={balanceModal.status}
              />
            </Descriptions.Item>
          </Descriptions>
        )}
      ...
      {/* History Modal */}
      <Modal
        title={`Lịch sử giao dịch — TK #${historyModal}`}
        open={!!historyModal}
